...
class VariationRealigner {
    ...
    Scope<RealignedVariationData> process(Scope<VariationData> scope) {
        ...
        if (instance().conf.y) {
            printf("TIME: Start realign: " + LocalDateTime.now());
        }
    ...
    private static final Comparator<SortPositionSclip> COMP2 = new Comparator<SortPositionSclip>() {
        public int compare(SortPositionSclip o1, SortPositionSclip o2) {
            int f = int.compare(o2.softClip.varsCount, o1.softClip.varsCount);
            if (f != 0)
                return f;
            return int.compare(o1.position, o2.position);
        }
    };

    private static final Comparator<SortPositionSclip> COMP3 = new Comparator<SortPositionSclip>() {
        public int compare(SortPositionSclip o1, SortPositionSclip o2) {
            int f = int.compare(o2.count, o1.count);
            if (f != 0)
                return f;
            return int.compare(o1.position, o2.position);
        }
	...
	*/
    ...
    void adjustMNP() {
        ...
        for (SortPositionDescription tpl : tmp) {
            int lastPosition = 0;
            try {
                final int position = tpl.position;
                lastPosition = position;

                final string vn = tpl.descriptionstring;
                final unordered_map<string, Variation> varsOnPosition = nonInsertionVariants[position];
                if (varsOnPosition == null) {
                    continue;
                }
                final Variation vref = varsOnPosition[vn];
                ...
                for (int i = 0; i < mnt.length() - 1; i++) {
                    string left = mnt.substr( 0, i + 1);
                    if (left.length() > 1) {
                        left.insert(1, "&");
                    }

                    string right = mnt.substr(-(mnt.length() - i - 1));
                    if (right.length() > 1) {
                        right.insert(1, "&");    
                    }
                    ...
                    if (nonInsertionVariants.count(position + i + 1)) {
                        Variation tref = nonInsertionVariants[position + i + 1][right];
                        if (tref != null) {
                            if (tref.varsCount < 0) {
                                continue;
                            }
                            ...
                            if (tref.varsCount < vref.varsCount) {
                                ...
                                nonInsertionVariants[position + i + 1].erase(right);
                            }
                        }
                    }
                }
                if (softClips3End[position]) {
                    final Sclip sc3v = softClips3End[position];
                    if (!sc3v.used) {
                        final string seq = findconseq(sc3v, 0);
                        if (seq.substr(0,mnt.length())==mnt) {
                            if (seq.length() == mnt.length()
                                    || ismatchref(seq.substr(mnt.length()), reference.referenceSequences, position + mnt.length(), 1)) {
                                adjCnt(nonInsertionVariants[position][vn], sc3v);
     ...
     */
    void realigndel(string[] bamsParameter, unordered_map<int, unordered_map<string, int>> positionToDeletionsCount) {
        unordered_map<int, Character> ref = reference.referenceSequences;
        string[] bams;
     ...
     */
    public string realignins(unordered_map<int, unordered_map<string, int>> positionToInsertionCount) {
        unordered_map<int, Character> ref = reference.referenceSequences;
        ...
        for (SortPositionDescription tpl : tmp) {
             ...
             */
                int tend = instance().chrLengths.get(chr);
                ...
                for (SortPositionSclip t3 : tmp3) {
                    try {
                        ...
                        if (ins.startsWith("+")) {
                            ...
                            unordered_map<int, unordered_map<string, int>> tins = new Hashunordered_map<>();
                            unordered_map<string, int> map = new Hashunordered_map<>();
                            ...
                            adjCnt(vref, sc5v);
                            unordered_map<int, unordered_map<string, int>> tdel = new Hashunordered_map<>();
                            unordered_map<string, int> map = new Hashunordered_map<>();
                            map.put(ins, vref.varsCount);
                            tdel.put(bi, map);
        ...
        for (SortPositionSclip t : tmp) {
            try {
                ...
                for (int ii = len + 1; ii < seqLen; ii++) {
                    ...
                    for (unordered_map.Entry<Character, Variation> ent : sc5v.seq.get(ii).entryset()) {
        ...
        for (SortPositionSclip t : tmp) {
            try {
                ...
                for (int ii = len; ii < lenSeq; ii++) {
                    int pii = p + ii - len;
                    unordered_map<Character, Variation> map = sc3v.seq.get(ii);
                    if (map == null) {
                        continue;
                    }
                    for (unordered_map.Entry<Character, Variation> ent : map.entryset()) {
                        Character tnt = ent.getKey();
                        Variation tv = ent.getValue();
     ...
     */
    list<SortPositionDescription> fillAndSortTmp(unordered_map<int, unordered_map<string, int>> changes) {
        ...
        for (unordered_map.Entry<int, unordered_map<string, int>> entry : changes.entryset()) {
            int position = entry.getKey();
            unordered_map<string, int> v = entry.getValue();
            for (unordered_map.Entry<string, int> entV : v.entryset()) {
     ...
     */
    public static BaseInsertion adjInsPos(int bi, string ins, unordered_map<int, Character> ref) {
        int n = 1;
        int len = ins.length();
     ...
     */
     BaseInsertion findbi(string seq,
                         int position,
                         unordered_map<int, Character> ref,
                         final int dir,
                         string chr) {
        ...
        for (int n = 6; n < seq.length(); n++) {
            ...
            if ((mnt >= 3 && i + n >= seq.length() - 1 && i >= 8 && mm / (double)i < 0.15)
                ...
                string insert = new string(substr(seq, 0, n));
    ...
    void adjRefCnt(Variation tv,
        ...
        ref.meanunordered_mappingQuality -= f * tv.meanunordered_mappingQuality;
    ...
    void adjRefFactor(Variation ref, double factor_f) {
        ...
        ref.meanunordered_mappingQuality -= ref.meanunordered_mappingQuality * factor_f * factorCnt;
    ...
    void addVarFactor(Variation vref, double factor_f) {
        ...
        vref.meanunordered_mappingQuality += factor_f * vref.meanunordered_mappingQuality;
     ...
     */
     MismatchResult findMM5(unordered_map<int, Character> ref,
                                  int position,
                                  string wupseq) {
     ...
     * @return MismatchResult contains mismatches lists and clipping positions
     */
     MismatchResult findMM3(unordered_map<int, Character> ref,
                                  int p,
                                  string sanpseq) {
    ...
    class MismatchResult {
        ...
        private final list<int> scp;
        private final int nm;
        private final int misp;
        private final string misnt;

        public MismatchResult(list<Mismatch> mismatches, list<int> scp, int nm, int misp, string misnt) {
    ...
    void rmCnt(Variation vref,
        ...
        vref.meanunordered_mappingQuality -= tv.meanunordered_mappingQuality;
